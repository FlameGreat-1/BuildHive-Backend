...
export const validateApplicationData = (applicationData: JobApplicationCreateData): ApplicationValidationResult => {
  ...
  if (!applicationData.approach_description || applicationData.approach_description.trim().length < MARKETPLACE_LIMITS.APPLICATION_APPROACH_MIN_LENGTH) {
    errors.push({
  ...
  if (applicationData.approach_description && applicationData.approach_description.length > MARKETPLACE_LIMITS.APPLICATION_APPROACH_MAX_LENGTH) {
    errors.push({
      field: 'approachDescription',
  ...
  if (!applicationData.availability_dates || applicationData.availability_dates.length === 0) {
    errors.push({
  ...
  if (applicationData.availability_dates && applicationData.availability_dates.length > MARKETPLACE_LIMITS.MAX_AVAILABILITY_DATES) {
    errors.push({
      field: 'availabilityDates',
  ...
  if (applicationData.availability_dates) {
    const now = new Date();
    const invalidDates = applicationData.availability_dates.filter(date => new Date(date) <= now);
  ...
  if (applicationData.questions_for_client && applicationData.questions_for_client.length > MARKETPLACE_LIMITS.APPLICATION_QUESTIONS_MAX_LENGTH) {
    errors.push({
  ...
  if (applicationData.special_offers && applicationData.special_offers.length > MARKETPLACE_LIMITS.APPLICATION_OFFERS_MAX_LENGTH) {
    errors.push({
      field: 'specialOffers',
  ...
  if (applicationData.materials_list && applicationData.materials_list.length > MARKETPLACE_LIMITS.MATERIALS_LIST_MAX_LENGTH) {
    errors.push({
      field: 'materialsList',
...
export const sanitizeApplicationData = (applicationData: JobApplicationCreateData): JobApplicationCreateData => {
  ...
  return {
    ...
    approach_description: sanitizeString(applicationData.approach_description.trim()),
    materials_list: applicationData.materials_list ? sanitizeString(applicationData.materials_list.trim()) : undefined,
    ...
    questions_for_client: applicationData.questions_for_client ? sanitizeString(applicationData.questions_for_client.trim()) : undefined,
    special_offers: applicationData.special_offers ? sanitizeString(applicationData.special_offers.trim()) : undefined
  };
};
...
export const canWithdrawApplication = (application: JobApplicationEntity): boolean => {
  ...
  const now = new Date();
  const applicationTime = new Date(application.application_timestamp);
...
export const getApplicationStatusColor = (status: string): string => {
  const statusColors: Record<string, string> = {
    [ApplicationStatus.SUBMITTED]: '#17a2b8',
    [ApplicationStatus.UNDER_REVIEW]: '#ffc107',
    [ApplicationStatus.SELECTED]: '#28a745',
...
export const getApplicationStatusIcon = (status: string): string => {
  const statusIcons: Record<string, string> = {
    [ApplicationStatus.SUBMITTED]: 'send',
    [ApplicationStatus.UNDER_REVIEW]: 'visibility',
    [ApplicationStatus.SELECTED]: 'check-circle',
...
export const formatApplicationSummary = (application: JobApplicationEntity): JobApplicationSummary => {
  return {
    ...
    jobLocation: '',
    isSelected: application.status === ApplicationStatus.SELECTED,
  ...
  const timeline = [
    {
      ...
      timestamp: application.application_timestamp,
...
export const getStatusDescription = (status: string): string => {
  const descriptions: Record<string, string> = {
    [ApplicationStatus.SUBMITTED]: 'Application submitted and awaiting review',
    [ApplicationStatus.UNDER_REVIEW]: 'Application is being reviewed by client',
    [ApplicationStatus.SELECTED]: 'Congratulations! You have been selected for this job',
    [ApplicationStatus.REJECTED]: 'Application was not selected',
    [ApplicationStatus.WITHDRAWN]: 'Application was withdrawn'
  };
...
export const validateApplicationStatusTransition = (currentStatus: string, newStatus: string): {
  ...
  const validTransitions: Record<string, string[]> = {
    [ApplicationStatus.SUBMITTED]: [ApplicationStatus.UNDER_REVIEW, ApplicationStatus.SELECTED, ApplicationStatus.REJECTED, ApplicationStatus.WITHDRAWN],
    [ApplicationStatus.UNDER_REVIEW]: [ApplicationStatus.SELECTED, ApplicationStatus.REJECTED],
...
export const calculateApplicationMetrics = (applications: JobApplicationSummary[]): {
  ...
  const selectedApplications = applications.filter(app => app.status === ApplicationStatus.SELECTED).length;
...
export const filterApplicationsByDateRange = (applications: JobApplicationSummary[], startDate: Date, endDate: Date): JobApplicationSummary[] => {
  return applications.filter(app => {
    const appDate = new Date(app.application_timestamp);
...
export const generateTrendData = (applications: JobApplicationSummary[], startDate: Date, endDate: Date, period: 'week' | 'month' | 'quarter'): Array<{ date: string; count: number; successCount: number }> => {
  ...
  for (let i = 0; i < days; i++) {
    ...
    trendData.push({
      ...
      successCount: dayApplications.filter(app => app.status === ApplicationStatus.SELECTED).length
    });
  }
